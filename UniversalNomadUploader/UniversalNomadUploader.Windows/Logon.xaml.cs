...
namespace UniversalNomadUploader
{
  ...
  public sealed partial class Logon : Page
  {
    ...
    private async void logon_Click(object sender, RoutedEventArgs e)
    {
      ...
      //    try
      //    {
      //        await Ignite.SQLUtils.UserUtil.InsertUser(new User() { Username = Username.Text, SessionID = Session });
      //        await Ignite.SQLUtils.UserUtil.UpdateUser(await Ignite.APIUtils.UserUtil.GetProfile());
      //    }
    ...
    {
      if (GlobalVariables.SelectedServer == ServerEnum.UAT1)
      {
        GlobalVariables.SelectedServer = ServerEnum.Live;
        Live.IsChecked = false;
        Live.IsEnabled = true;
